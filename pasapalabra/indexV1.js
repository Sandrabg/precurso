...
function getQuestionsStatus(status)
{
    ...
    for (element in questions){
        if (questions[element] === status) result++;
    }
}

function printResult(){
    var resultOK = getQuestionsStatus(0);
    console.log("Juego Terminado Resultado:");
    console.log(`Preguntas Acertadas: ${resultOK}`);
    console.log(`Preguntas Falladas: ${ (questions.filter(question => question.status === 2)).length}`);
    console.log(`Preguntas Sin Responder: ${ (questions.filter(question => question.status === 0)).length}`);
...
function continueQuestions(){
    return (((questions.filter(question => question.status === 0)).length > 0) ? true : false);
}
...
function game(){
    ...
    do{    
        for (element in questions){
            if (questions[element].status === 0){
                answer =  (questions[element].answer).toUpperCase();
                resultQuestion = (printQuestion(element)).toUpperCase();
                status = TratarResult(resultQuestion, answer); 
                if (status  === -1){
                    console.log("Bye!");
                    break;
                }    
                questions[element].status  = status;
            }
        }
